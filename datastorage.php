...
class News
{
    ...
    public function getMediaData($topic1, $topic2)
    {
        $info = array();

        foreach ($this->data['data']['news'] as $value) {

            if($topic1 == $value['topic'] && $topic2 == $value['language'] )
            {
                $obj = new stdClass();
    ...
    public function getNumberOfData($topic1, $topic2)
    {
        $count = 0;
    ...
    public function displayMediaData($temp, $topic, $numdata, $index)
    {
        //$number_of_data = $this->getNumberOfData($topic);
        ...
        if($temp != null)
        {
            ...
            echo '<h4>Data dengan topik '.$topic.' yang anda cari tidak ada pada halaman ini</h4>';
            echo '<div>';
        }
...
class MediaData
{
    ...
    function __construct($jsonfile) {
        ...
        $cxContext = stream_context_create($aContext);
        $sFile = file_get_contents($jsonfile, False, $cxContext);
		$this->saveJsonResult("cache.json",$sFile);
        $data = json_decode($sFile, true);
        $this->data = $data;
    }
	...
	public function saveJsonResult($filename, $data)
    {
